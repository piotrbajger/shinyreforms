...
Form submission can be detected through the `submissionEvent` in the server
function as shown below. The `validate` method calls validators on each
input and returns `TRUE` if all validators for all inputs returned `TRUE`.
